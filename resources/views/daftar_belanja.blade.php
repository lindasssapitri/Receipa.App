...
<head>
    ...
    <title>Recepia - Daftar Belanja</title>
...
<body class="min-h-screen text-gray-800">
    ...
    <main class="max-w-7xl mx-auto px-4 md:px-8 py-6 space-y-8">
        ...
        <div class="flex flex-wrap justify-center items-center gap-4 py-2">
            ...
            <!-- Tombol Aktif Halaman Ini (Daftar Belanja) -->
            <a href="{{ route('daftar.belanja') }}" class="flex items-center justify-between w-64 bg-[#B56C00] text-white px-5 py-4 rounded-2xl shadow-inner border-2 border-white/30 font-bold text-sm tracking-wide transition">
                <span>Buat daftar belanja</span>
                <i class="fa-solid fa-chevron-right text-xs"></i>
            </a>
            <a href="{{ route('resep.tersimpan') }}" class="flex items-center justify-between w-64 bg-[#78B000] hover:bg-[#669600] text-white px-5 py-4 rounded-2xl shadow-md font-bold text-sm tracking-wide transition transform active:scale-95">
                <span>Resep Tersimpan</span>
                <i class="fa-solid fa-chevron-right text-xs"></i>
            </a>
        </div>

        <!-- 4. KONTEN UTAMA: DAFTAR BELANJA -->
        <section class="space-y-6 flex flex-col items-center w-full">
            <h2 class="text-xl md:text-2xl font-black text-center text-white tracking-wide uppercase">Daftar Belanja</h2>
            
            <!-- Bar Pencarian "Search Anything" Sesuai Figma -->
            <div class="w-full max-w-xl bg-white rounded-xl shadow-md flex items-center px-4 py-3 border border-gray-200">
                <i class="fa-solid fa-magnifying-glass text-gray-400 mr-3 text-sm"></i>
                <input type="text" placeholder="Search Anything" class="w-full bg-transparent text-xs outline-none font-sans text-gray-700">
            </div>

            <!-- KOMPONEN TAMBAHAN: Form Tambah & Checklist Note Belanja -->
            <div class="w-full max-w-xl bg-white rounded-3xl p-6 md:p-8 shadow-xl border border-gray-100 space-y-6">
                <!-- Mini Form Input Belanjaan Baru -->
                <div class="space-y-2">
                    <label class="text-xs font-bold text-gray-700 tracking-wide block">Tambah Bahan Belanjaan Baru:</label>
                    <div class="flex gap-2">
                        <input type="text" id="todo-input" placeholder="Contoh: 500gr Daging Sapi Segar..." class="w-full bg-gray-50 border border-gray-300 rounded-xl px-4 py-2.5 text-xs outline-none focus:border-orange-500 font-sans">
                        <button onclick="addTodo()" class="bg-[#FF7A00] hover:bg-orange-600 text-white font-bold text-xs px-5 py-2.5 rounded-xl transition shadow-md">
                            Tambah
                        </button>
                    </div>
                </div>
                ...
                <!-- List Bahan Belanjaan dengan Ceklis (Interaktif Sederhana via JS) -->
                <div class="space-y-3">
                    <span class="text-xs font-bold text-gray-500 tracking-wider uppercase block">Bahan Belanjaan Hari Ini:</span>
                    <ul id="todo-list" class="space-y-2 text-xs font-sans">
                        <li class="flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                            <input type="checkbox" class="w-4 h-4 accent-orange-500 cursor-pointer">
                            <span class="text-gray-700 font-semibold">Bumbu Rendang Instan (2 Bungkus)</span>
                        </li>
                        <li class="flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                            <input type="checkbox" class="w-4 h-4 accent-orange-500 cursor-pointer" checked>
                            <span class="text-gray-400 line-through font-semibold">Kulit Lumpia (1 Pack)</span>
                        </li>
                        <li class="flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                            <input type="checkbox" class="w-4 h-4 accent-orange-500 cursor-pointer">
                            <span class="text-gray-700 font-semibold">Iga Sapi Potong (1 kg)</span>
                        </li>
                        <li class="flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                            <input type="checkbox" class="w-4 h-4 accent-orange-500 cursor-pointer">
                            <span class="text-gray-700 font-semibold">Paprika Merah & Hijau</span>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <!-- Script Sederhana untuk fungsionalitas tambah item belanja -->
    <script>
        function addTodo() {
            ...
            if(input.value.trim() !== '') {
                ...
                li.className = "flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200 transform scale-95 opacity-0 transition duration-300 ease-out";
                li.innerHTML = `
                    <input type="checkbox" class="w-4 h-4 accent-orange-500 cursor-pointer">
                    <span class="text-gray-700 font-semibold">${input.value}</span>
                `;
                list.appendChild(li);
